@extends('admin.layouts.app', ['title' => 'Edit Social Embed'])
...
@section('content')
    <div>
        ...
        <form action="{{ route('admin.social-embeds.update', $socialEmbed) }}" method="POST">
            @csrf
            @method('PUT')
                        ...
                        <div class="card-body">
                            <div class="form-group">
                                ...
                                <input type="text" name="title" class="form-control"
                                    value="{{ old('title', $socialEmbed->title) }}" required>
                            </div>

                            <div class="form-group">
                                <label>Source</label>
                                <input type="text" name="source" class="form-control"
                                    value="{{ old('source', $socialEmbed->source) }}" required>
                            </div>

                            <div class="form-group mt-3">
                                <label>Embed Body</label>
                                <textarea id="embedBody" name="body" class="form-control" rows="10" required>{{ old('body', $socialEmbed->body) }}</textarea>
                            </div>

                            <div class="mt-3">
                                <button type="submit" class="btn btn-primary">Update Social Embed</button>
                            </div>
                        </div>
                    </div>
                </div>
