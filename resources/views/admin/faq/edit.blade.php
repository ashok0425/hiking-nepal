            ...
            <form action="{{ route('admin.faqs.update', $faq->id) }}" enctype="multipart/form-data" method="POST">
                ...
                <div class="row alugn-items-center">
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" class="form-control" name="question" placeholder="Enter name here" required
                            value="{{ $faq->question }}">
                    </div>

                    <div class="form-group col-md-6">
                        <label> Answer</label>
                        <input type="text" class="form-control" name="answer" placeholder="Enter title here" required
                            value="{{ $faq->answer }}">
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        ...
                        <select name="package_id" id="" class="form-control">
                            ...
                            @foreach ($packages as $package)
                                <option value="{{ $package->id }}" @if ($package->id == $faq->package_id) selected @endif>
                                    {{ $package->name }}</option>
                            @endforeach
                        </select>
                    </div>
