<section class="hero">
    <div class="hero-cloud-r1 animate__animated animate__slideInRight"></div>
    <div class="hero-cloud-l1 animate__animated animate__slideInLeft"></div>

    <h1 class="animate__animated animate__slideInUp text-white">Hiking Nepal</h1>
    <form action="{{ route('deals') }}" method="GET" class="animate__animated animate__zoomIn">
        <div class="input-group mb-3">
            <span class="input-group-text text-primary" id="basic-addon1">
                <i class="fas fa-search"></i>
            </span>
            <input name="search" class="form-control ps-0 border-start-0" type="text"
                placeholder="Find your trip..." required>
        </div>
    </form>

    <div class="mount-1 animate__animated animate__slideInUp"></div>
    <div class="mount-2 animate__animated animate__slideInUp"></div>
    <div class="mount-3 animate__animated animate__slideInUp"></div>

    <div class="hero-cloud-l2 animate__animated animate__slideInLeft"></div>
    <div class="hero-cloud-center animate__animated animate__slideInRight"></div>

</section>
